...
│                         INGESTION LAYER                             │
│                                                                     │
│  events.coinpedia.org ──► ConferenceDiscoveryService               │
│  Bitcoin RSS Feeds    ──► RSSPoller (6 default feeds)              │
│  YouTube Channels     ──► ChannelScanner (APScheduler every 6h)    │
│                                  │                                  │
│                          YouTubeChannel DB                          │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │
...
│                                                                     │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  │
│  │  Whisper │  │ Deepgram │  │SmallestAI│  │  Gemini STT  NEW │  │
│  │ (local)  │  │ Nova-2   │  │  Pulse   │  │ (Files API + LLM)│  │
│  └──────────┘  └──────────┘  └──────────┘  └──────────────────┘  │
│                                                                     │
│  All providers produce: { text, segments[{start, end, speaker}] }  │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │
┌──────────────────────────────────▼──────────────────────────────────┐
...
│                                                                     │
│  ┌─────────────────────┐    ┌──────────────────────────────────┐   │
│  │  CorrectionService  │    │    SummarizerService  IMPROVED   │   │
│  │  Bitcoin-domain     │    │  Structured JSON output:         │   │
│  │  ASR error fixing   │    │  abstract, key_topics, speakers, │   │
│  │  (OpenAI / Gemini)  │    │  key_moments, bitcoin_terms      │   │
│  └─────────────────────┘    └──────────────────────────────────┘   │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │              TranslationService  NEW                         │  │
│  │  Sarvam AI  →  Hindi, Tamil, Telugu, Kannada, Malayalam,    │  │
│  │                Bengali, Gujarati, Marathi, Punjabi, Odia    │  │
│  │  OpenAI/Gemini → Spanish, French, German, Portuguese,       │  │
...
│                                                                     │
│  Supabase PostgreSQL                                                │
│  ├── youtube_channels   (name, channel_id, category, priority)     │
│  ├── youtube_videos     (channel, title, duration, status)         │
│  ├── ingestion_runs     (video_id, provider, status, timestamps)   │
│  └── transcripts        (raw, corrected, summary, translations)    │
│                                                                     │
│  Local filesystem                                                   │
│  ├── audio/competency/  *.mp3 — downloaded audio for benchmarking  │
│  └── metadata/          rss_state.json — RSS dedup state           │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │
┌──────────────────────────────────▼──────────────────────────────────┐
│                           API LAYER                                 │
│                                                                     │
│  FastAPI — server.py (lifespan startup: APScheduler)               │
│                                                                     │
│  /health                    — server status                         │
│  /transcription/*           — queue, submit, view results          │
│  /ingestion/*               — channels, videos, runs, RSS, sched.  │
│  /translation/*   NEW       — text, transcript, languages          │
│  /conference/*    NEW       — discover, supported-languages        │
│                                                                     │
